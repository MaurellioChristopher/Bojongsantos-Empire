...
export function getSurplusItems(): SurplusItem[] {
  ...
  // Check if new bahan_baku items (bdg-15 to bdg-20) are present
  const hasBahanBakuExpanded = items.some((i) => i.id === 'surplus-bdg-15');
  if (!items.some((i) => i.id.startsWith('surplus-bdg')) || !hasActiveBdg || needsPhotoUpdate || !hasBahanBakuExpanded) {
