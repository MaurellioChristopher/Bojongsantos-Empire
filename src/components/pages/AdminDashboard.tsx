...
export function AdminDashboard() {
  ...
  if (!user || user.role !== 'admin') {
    return (
      <div className="min-h-[70vh] flex flex-col items-center justify-center p-6 text-center bg-[#FBF7F0]">
        <div className="w-16 h-16 rounded-full bg-[#FFFDF9] flex items-center justify-center text-[#7D6F64] mb-4 border border-[#EADECF] shadow-xs">
          <Shield size={32} />
        </div>
        <h2 className="text-display-md text-[#2C221D] mb-2">Akses Dibatasi</h2>
        <p className="text-body-apple text-[#7D6F64] max-w-sm mb-6">
    ...
    <div className="min-h-screen bg-[#FBF7F0] py-8 px-4 sm:px-8">
      <div className="apple-container">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
          <div>
            <h1 className="text-display-lg text-[#2C221D] mb-1">Admin Governance</h1>
        ...
        <div className="flex items-center bg-[#EFE8DD] p-1 rounded-full w-full max-w-md mb-8 overflow-x-auto border border-[#EADECF]/60">
          {tabs.map((tab) => (
            <button
              key={tab.id}
              ...
              className={`flex-1 py-1.5 px-3 rounded-full text-xs font-semibold tracking-tight transition-all flex items-center justify-center gap-1.5 whitespace-nowrap ${
                activeTab === tab.id
                  ? 'bg-[#FFFDF9] text-[#2C221D] shadow-xs'
        ...
        {activeTab === 'overview' && impact && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div className="card-apple-utility bg-[#FFFDF9] border border-[#EADECF] p-5 text-center">
                <div className="text-display-md font-semibold text-[#2C221D]">{users.length}</div>
                <div className="text-caption-apple text-[#7D6F64]">Total Akun Terdaftar</div>
              </div>
              <div className="card-apple-utility bg-[#FFFDF9] border border-[#EADECF] p-5 text-center">
              ...
              <div className="card-apple-utility bg-[#FFFDF9] border border-[#EADECF] p-5 text-center">
                <div className="text-display-md font-semibold text-[#2C221D]">{bookings.length}</div>
                <div className="text-caption-apple text-[#7D6F64]">Transaksi Tercatat</div>
              </div>
              <div className="card-apple-utility bg-[#FFFDF9] border border-[#EADECF] p-5 text-center">
                <div className="text-display-md font-semibold text-[#2E7D32]">{impact.totalCO2eSaved} kg</div>
                <div className="text-caption-apple text-[#7D6F64]">CO₂e Ditekan</div>
              </div>
            </div>

            <div className="card-apple-utility bg-[#FFFDF9] border border-[#EADECF] p-6">
              <h3 className="text-tagline text-[#2C221D] mb-4">Transaksi Terkini</h3>
              <div className="divide-y divide-[#EADECF]/60">
                {bookings.slice(0, 5).map((b) => (
                  <div key={b.id} className="py-3.5 flex items-center justify-between">
                    ...
                    <span className="badge-apple badge-apple-neutral text-xs bg-[#FAF7F2] text-[#7D6F64] border border-[#EADECF]">
        ...
        {activeTab === 'users' && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-4">
            <div className="relative">
              <Search size={18} className="absolute left-4 top-1/2 -translate-y-1/2 text-[#7D6F64]" />
              <input
                type="text"
                placeholder="Cari akun pengguna berdasarkan nama atau email..."
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                className="apple-search-pill bg-[#FFFDF9] border border-[#EADECF] text-[#2C221D]"
              />
            </div>

            <div className="card-apple-utility bg-[#FFFDF9] border border-[#EADECF] p-0 overflow-hidden">
              <div className="divide-y divide-[#EADECF]/60">
                {filteredUsers.map((u) => (
                  <div key={u.id} className="p-4 sm:px-6 flex items-center justify-between">
                    <div>
                      <div className="text-body-strong text-[#2C221D] flex items-center gap-2">
                        {u.name}
                        {u.id === user.id && (
                          <span className="text-xs text-[#7D6F64] font-normal">(Akun Anda)</span>
                        )}
                      </div>
                      <div className="text-caption-apple text-[#7D6F64]">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      {u.id !== user.id && (
                        <button
                          ...
                          className="p-1.5 text-[#D95327] hover:bg-[#FBEFEA] rounded-full transition-colors"
              ...
              <div className="divide-y divide-[#EADECF]/60">
                {bookings.map((b) => (
                  <div key={b.id} className="p-4 sm:px-6 flex items-center justify-between">
                    <div>
                      <div className="text-body-strong text-[#2C221D] mb-0.5">{b.surplusName}</div>
                      <div className="text-caption-apple text-[#7D6F64]">
                        Penerima: {b.recipientName} • Restoran: {b.providerBusinessName} • Jumlah: {b.quantity} kg
                      </div>
                      <div className="text-fine-print text-[#7D6F64] mt-1">
        ...
        {activeTab === 'complaints' && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div>
                <h3 className="text-tagline text-[#2C221D] mb-1">Pusat Layanan &amp; Keluhan Pengguna</h3>
                <p className="text-caption-apple text-[#7D6F64]">
                  Daftar laporan kendala dan tiket aduan dari Mitra Penyedia dan Penerima Manfaat.
                </p>
              </div>
              <div className="text-xs text-[#7D6F64] font-mono">
                Total {complaints.length} Tiket
              </div>
            </div>

            {complaints.length === 0 ? (
              <div className="card-apple-utility bg-[#FFFDF9] border border-[#EADECF] p-12 text-center">
                ...
                <h4 className="text-body-strong text-[#2C221D] mb-1">Tidak Ada Keluhan Tertunda</h4>
                <p className="text-caption-apple text-[#7D6F64] m-0">Semua laporan kendala pengguna telah berhasil diselesaikan.</p>
              </div>
            ) : (
              <div className="space-y-4">
                {complaints.map((c) => (
                  <div key={c.id} className="card-apple-utility bg-[#FFFDF9] p-5 sm:p-6 border border-[#EADECF]">
                        ...
                        <div className="flex items-center gap-2 mb-1">
                          <span className={`text-[10px] uppercase font-bold font-mono px-2 py-0.5 rounded-full ${
                            c.status === 'resolved'
                              ...
                              : 'bg-[#FBEFEA] text-[#D95327] border border-[#F2D7CD]'
                          }`}>
                            {c.status === 'resolved' ? 'SELESAI' : c.status === 'in_progress' ? 'SEDANG DITANGANI' : 'BARU'}
                          </span>
                          <span className="text-[11px] font-mono text-[#7D6F64]">
                            {formatDateTime(c.createdAt)}
                          </span>
                        </div>
                        <h4 className="text-base font-semibold text-[#2C221D] m-0">{c.subject}</h4>
                        <div className="text-xs text-[#7D6F64] mt-1">
                          Dari: <strong className="text-[#2C221D]">{c.userName}</strong> ({c.userRole.toUpperCase()} • {c.userEmail})
                        </div>
                      </div>
                    ...
                    <p className="text-xs sm:text-sm text-[#2C221D] bg-[#FAF7F2] p-3.5 rounded-xl border border-[#EADECF]/60 m-0 leading-relaxed">
                      {c.message}
                    </p>

                    <div className="flex items-center justify-between gap-3 mt-4 pt-3 border-t border-[#EADECF]/60 text-xs">
                      <span className="text-[#7D6F64]">Ubah Status Tiket:</span>
        ...
        {activeTab === 'governance' && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
            <div>
              <h3 className="text-tagline text-[#2C221D] mb-1">Tata Kelola Standar Mutu &amp; Dampak ESG</h3>
              <p className="text-caption-apple text-[#7D6F64]">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
              <div className="card-apple-utility bg-[#FFFDF9] p-6 border border-[#EADECF] flex flex-col justify-between">
                <div>
                  <div className="w-10 h-10 rounded-xl flex items-center justify-center bg-[#FBEFEA] text-[#D95327] border border-[#F2D7CD] mb-3">
                    <BookOpen size={20} />
                  </div>
                  <h4 className="text-base font-semibold text-[#2C221D] mb-1">Standar Mutu &amp; Regulasi</h4>
                  <p className="text-xs text-[#7D6F64] leading-relaxed mb-4">
              ...
              <div className="card-apple-utility bg-[#FFFDF9] p-6 border border-[#EADECF] flex flex-col justify-between">
                <div>
                  ...
                  <h4 className="text-base font-semibold text-[#2C221D] mb-1">Laporan &amp; Target Dampak ESG</h4>
                  <p className="text-xs text-[#7D6F64] leading-relaxed mb-4">
        ...
        {activeTab === 'microservices' && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
            <div>
              <h3 className="text-tagline text-[#2C221D] mb-1">Status Operasional Microservices</h3>
              <p className="text-caption-apple text-[#7D6F64]">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {services.map((svc) => (
                <div key={svc.service} className="card-apple-utility bg-[#FFFDF9] border border-[#EADECF] p-5 flex items-start justify-between">
                  <div className="flex items-start gap-3.5">
                    ...
                    <div>
                      <div className="text-body-strong text-[#2C221D] mb-0.5">{svc.service}</div>
                      <div className="flex items-center gap-2 text-xs text-[#2E7D32] font-medium">
                        <span className="w-2 h-2 rounded-full bg-[#2E7D32] animate-pulse" />
                        100% Operasional
                      </div>
                      <div className="text-fine-print text-[#7D6F64] mt-1.5">
            ...
            <div className="card-apple-utility bg-[#FFFDF9] border border-[#EADECF] p-6">
              <h4 className="text-body-strong text-[#2C221D] mb-3">Katalog Endpoint Terintegrasi</h4>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs font-mono text-[#2C221D]">
                <div className="bg-[#FAF7F2] border border-[#EADECF]/60 p-2.5 rounded-[8px] flex items-center justify-between">
